...
export default function Welcome() {
  ...
  const resolveIdeaStatus = useCallback((idea: Idea) => {
    const preferredRoleKey = (Object.keys(PREFERRED_LABEL_MAP) as Array<keyof Idea['team']>).find(
      key => {
        const label = PREFERRED_LABEL_MAP[key];
        return label === idea.preferredPart || key === idea.preferredPart;
      }
    );
          ...
          <>
            <IdeaTable>
              <IdeaHeaderRow>
                <span>순번</span>
                <span>아이디어 내용</span>
                <span style={{ textAlign: 'right' }}>지원 현황</span>
                <span style={{ textAlign: 'right' }}>모집 상태</span>
              </IdeaHeaderRow>
              {paginatedIdeas.map((idea, index) => (
                <IdeaItem key={idea.id} idea={idea} index={startIndex + index + 1} />
              ))}
            </IdeaTable>
